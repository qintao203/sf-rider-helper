    ...
    private void toggleOnline() {
        ...
        if (isOnline) {
            ...
            if (s != null && !s.isActive()) {
                if (isAccEnabled()) s.startGrabLoop();
                else try { startActivity(new Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS)); } catch (Exception ignored) {}
            }
        } else {
            onlineBtn.setText("⚪ 离线");
            onlineBtn.setTextColor(ThemeEngine.TEXT_DISABLED);
            onlineBtn.setBackground(ThemeEngine.roundedBg(0xFF33334D, dp(14)));
            if (s != null && s.isActive()) s.stopGrabLoop();
        }
    }
