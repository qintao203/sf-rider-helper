...
public class GrabAccessibilityService extends AccessibilityService {
    ...
    public void startGrabLoop() {
        ...
        // 启动前台服务（捕获Android 14+限制异常）
        try {
            Intent fi = new Intent(this, GrabForegroundService.class);
            ...
            Log.e(TAG, "Foreground service start failed: " + e.getMessage());
            // Android 14+可能限制前台服务，降级为普通服务
            try { startService(new Intent(this, GrabForegroundService.class)); } catch (Exception ignored) {}
        }

        // 启动守护进程（捕获IllegalStateException）
        try {
            if (Build.VERSION.SDK_INT < 31) {
                startService(new Intent(this, GuardService.class));
            } else {
                // Android 12+ 需要foreground service type
